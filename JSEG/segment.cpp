...
int segment(unsigned char *rmap0,unsigned char *cmap,int N,int nt,int ny,int nx,int dim,int NSCALE,int tt)
{
  float MINRSIZE[5] = {0,0,0,0,0};
  int scale[5],offset[5],step[5],MAXSCALE,MINSCALE=0,*count,TR,i,j,k,l;
  int oldTR,extraTR,*reg,*reg2,*convert,datasize,autoscale=0;
  short *rmap;
...
// XXXXXXXXXX REMOVE LIMITATION
  if (i==0) { abort(); }        // "minimum image size 64x64\n")
  if (nt==1)
  {
  ...
  {
    i = MINSCALE-1;
    TR=segment2(rmap0,rmap,i,cmap,N,nt,ny,nx,tt,oldTR,MINRSIZE,offset,step);
  }
...
int segment2(unsigned char *rmap0,short *rmap,int i,unsigned char *cmap,int N,int nt,
    int ny,int nx,int tt,int oldTR,float *MINRSIZE,int *offset,int *step)
{
...
{
  float *J,*JT=NULL,**threshJ1,**threshJ2;
...
int merge1(unsigned char *rmap,unsigned char *cmap,int N,int nt,int ny,int nx,int TR,
    float threshcolor)
{
  int it,iy,ix,ir,jr,i,mini=0,minj=0,newtr,*npt,npttotal;
  float *currentJ,*mergeJ,mindist,**distnpt,**distcolor,**distJ,**unused=NULL,**P;
...
{
  int it,iy,ix,ir,*npt,npttotal,jr,i,mini=0,minj=0,newtr;
